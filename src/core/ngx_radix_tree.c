...
ngx_radix_tree_create(ngx_pool_t *pool, ngx_int_t preallocate)
{
    ...
    mask = 0;
    inc = 0x80000000; /* bin: 10000000 00000000 00000000 00000000 */
    ...
    uintptr_t value)
{
    ...
    bit = 0x80000000; /* bin: 10000000 00000000 00000000 00000000 */

    node = tree->root;
    next = tree->root;

    while (bit & mask) { // 这里是判断哪个bit被允许
        if (key & bit) {
            next = node->right;
        } else {
            next = node->left;
    ...
    if (next) { // 这种情况: node == next, 表示当前 key & mask 节点已经存在
        if (node->value != NGX_RADIX_NO_VALUE) { // 如果节点已经有数据, 返回繁忙错误
            return NGX_BUSY;
        }
